...
3. **定期删除**：**默认每隔100ms「随机」**从redis中取出**一定数量的 key** 进行检查，并删除其中的过期key。如果「过期 key 」占比**大于 25%**，则再取一次。 Redis 为了**保证**定期删除**不会出现循环过度**，导致**线程卡死**现象，为此增加了定期删除循环**流程的时间上限**，默认不会超过 25ms。
